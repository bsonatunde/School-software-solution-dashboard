...
export async function POST(request: NextRequest) {
  try {
    ...
    const { firstName, lastName, email, specializations, subjects = [] } = body;
    
    if (!firstName || !lastName || !email || !specializations || !Array.isArray(specializations) || specializations.length === 0) {
      return NextResponse.json(
        { success: false, error: 'Missing required fields: firstName, lastName, email, and at least one specialization.' },
        { status: 400 }
      );
    }
